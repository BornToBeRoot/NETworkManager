...
- New **Speed Test** widget to measure download/upload speed, latency, and jitter against [`speed.cloudflare.com`](https://speed.cloudflare.com/). The test is user-initiated and shows download (Mbps), upload (Mbps), latency (ms), jitter (ms), ISP, client location, and Cloudflare PoP. A privacy disclaimer is shown on first use. [#3440](https://github.com/BornToBeRoot/NETworkManager/pull/3440)
